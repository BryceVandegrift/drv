...
drv_newref()
{
    return calloc(1, sizeof(drv_ref));
}
...
static bool
drv_bookequal(const char *a, const char *b, bool short_match)
{
    for (size_t i = 0, j = 0; ; ) {
...
static bool
drv_book_matches(const drv_book *book, const char *s)
{
    return drv_bookequal(book->name, s, false) ||
        drv_bookequal(book->abbr, s, false) ||
        drv_bookequal(book->name, s, true);
}

static int
drv_book_fromname(const char *s)
{
    for (int i = 0; i < drv_books_length; i++) {
        const drv_book *book = &drv_books[i];
        if (drv_book_matches(book, s)) {
            return book->number;
        }
    }
    return 0;
...
static int
drv_scanbook(const char *s, int *n)
{
    int i;
    int mode = 0;
    for (i = 0; s[i]; i++) {
...
drv_parseref(drv_ref *ref, const char *ref_str)
{
    ...
    int n = 0;
    if (drv_scanbook(ref_str, &n) == 1) {
        // 1, 2, 3, 3a, 4, 5, 6, 8, 9
        char *bookname = strndup(ref_str, n);
        ref->book = drv_book_fromname(bookname);
        free(bookname);
        ref_str = &ref_str[n];
    } else if (ref_str[0] == '/') {
        // 7
