/*
drv: Read the Word of God from your terminal
...
const char *
usage = "usage: drv [flags] [reference...]\n"
    "\n"
    "Flags:\n"
...
main(int argc, char *argv[])
{
    ...
    for (int opt; (opt = getopt(argc, argv, "A:B:CeplWh")) != -1; ) {
        ...
        case 'A':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "drv: invalid flag value for -A\n\n%s", usage);
                return 1;
            }
            break;
        case 'B':
            ...
            if (endptr[0] != '\0') {
                fprintf(stderr, "drv: invalid flag value for -B\n\n%s", usage);
                return 1;
            }
            break;
        case 'C':
        ...
        case 'h':
            ...
            return 0;
        case '?':
            fprintf(stderr, "drv: invalid flag -%c\n\n%s", optopt, usage);
            return 1;
        }
    }
    ...
    if (argc == optind) {
        using_history();
        while (true) {
            char *input = readline("drv> ");
            if (input == NULL) {
                break;
            }
            add_history(input);
            drv_ref *ref = drv_newref();
            int success = drv_parseref(ref, input);
            free(input);
            if (success == 0) {
                drv_render(ref, &config);
            }
            drv_freeref(ref);
        }
    } else {
        ...
        drv_ref *ref = drv_newref();
        int success = drv_parseref(ref, ref_str);
        free(ref_str);
        if (success == 0) {
            drv_render(ref, &config);
        }
